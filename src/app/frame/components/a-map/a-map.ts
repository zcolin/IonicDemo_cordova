...
import {AdvancedInfoWindowOption, CircleOption, ControlBarPluginOption, IconOption, LineOption, LocationPluginOption, MakerOption, MapTypePluginOption, PolygonOption, ScalePluginOption, SearchNearOption, ToolBarPluginOption} from './a-map.option';

@Component({
    selector: 'a-map',
    templateUrl: 'a-map.html',
    styleUrls: ['a-map.scss']
})
export class AMapComponent implements AfterContentInit {
    private _mapView;
    ...
    getMaker(option: MakerOption, clickCallBack?: (e) => void) {
        const icon = option.icon && option.icon instanceof IconOption ? new AMap.Icon({
    ...
    private covert(resultList: any[],
        ...
        callback: () => void) {
        const size = positionList.length - index > 40 ? 40 : positionList.length - index;
        if (size > 0) {
            const array = positionList.slice(index, index + size);
            AMap.convertFrom(array, type || 'gps', (status, result) => {
                if (status === 'complete' && result.info === 'ok') {
                    const locations: number[][] = result.locations.map((value) => [value.lat, value.lng]);
