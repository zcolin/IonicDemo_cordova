...
export class BaiduBaseLayer extends BaseLayer {
    ...
    public getBaseMaps(): Promise<any[]> {
        return loadModules([
            ...
            const BaiduTileLayer = BaseTileLayer.createSubclass({
                properties: {
                    urlTemplate: null
                },
                getTileUrl: function (level, row, col) {
                    ...
                    const numX = col - offsetX, numY = (-row) + offsetY;
                    const num = (col + row) % 8 + 1;
                    return this.urlTemplate.replace('{subDomain}', num)
                               .replace('{level}', level)
                               .replace('{col}', numX)
                               .replace('{row}', numY);
                }
            ...
            const baseLayerImg = new Basemap({
                baseLayers: [
                    new BaiduTileLayer(
                        {
