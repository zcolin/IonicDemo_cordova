 ...
 */
@Injectable()
export class BaiduBaseLayer extends BaseLayer {
    ...
    public getBaseMaps(): Promise<any[]> {
        return loadModules([
            ...
            "dojo/domReady!"
        ]).then(([Basemap, BaseTileLayer, SpatialReference, TileInfo, Extent]) => {
            let spatialReference = new SpatialReference({ wkid: 3857 });
            let BaiduTileLayer = BaseTileLayer.createSubclass({
                properties: {
                    urlTemplate: null
                },
                getTileUrl: function (level, row, col) {
                    let zoom = level - 1;
                    let offsetX = Math.pow(2, zoom);
                    let offsetY = offsetX - 1;
                    let numX = col - offsetX, numY = (-row) + offsetY;
                    let num = (col + row) % 8 + 1;
                    return this.urlTemplate.replace("{subDomain}", num).replace("{level}", level).replace("{col}", numX).replace("{row}", numY);
                }
            });
            ...
            let baseLayerVec = new Basemap({
                baseLayers: new BaiduTileLayer(
                    {
                        urlTemplate: "http://online{subDomain}.map.bdimg.com/tile/?x={col}&y={row}&z={level}&qt=tile&styles=pl&udt=20180505",
                    }
                ),
                spatialReference: spatialReference,
                title: '街道地图',
                ...
                tileInfo: tileInfo,
                //fullExtent: new Extent(-20037508.3427892, -20037508.3427892, 20037508.3427892, 20037508.3427892),
            });

            let baseLayerImg = new Basemap({
                baseLayers: [
                    new BaiduTileLayer(
                        {
                            urlTemplate: "http://shangetu{subDomain}.map.bdimg.com/it/u=x={col};y={row};z={level};v=009;type=sate&fm=46&udt=20180505",
                        }
                    ),
                    new BaiduTileLayer(
                        {
                            urlTemplate: "http://online{subDomain}.map.bdimg.com/tile/?x={col}&y={row}&z={level}&qt=tile&styles=sl&udt=20180505",
                        }
                    )],
                spatialReference: spatialReference,
                title: '影像地图',
            ...
            return [baseLayerVec, baseLayerImg];
        });
    }
    getMaxZoom() {
