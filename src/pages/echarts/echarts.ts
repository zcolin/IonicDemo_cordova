...
import { DbapiProvider } from '../../providers/dbapi/dbapi';
...
export class EchartsPage {
  constructor(public navCtrl: NavController, public navParams: NavParams,private db:DbapiProvider) {
  }
  ...
  createBarCharts() {
    this.db.getBarData(function(response){
      ...
      let myChart = ECharts.init(document.getElementById('barEcharts') as HTMLDivElement);
      // 指定图表的配置项和数据
      let option = {
        title:{
  ...
  createPieChart(){
    this.db.getPieChartDAta(function(response){
      let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('pieEcharts') as HTMLDivElement);
      let option={
        title:{
          text:responseData.title,
          subtext:'',
  ...
  createGroupBarEchart(){
    this.db.getGroupBarEcharts(function(response){
      let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('groupBarEcharts') as HTMLDivElement);
      let option={
        title:{
          text:responseData.title,
          subtext:'',
  ...
  createStackBarEcharts(){
    this.db.getStackBarEcharts(function(response){
      let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('stackBarEcharts') as HTMLDivElement);
      let option={
        title:{
          text:responseData.title,
          subtext:'',
  ...
  createLineChart(){
    this.db.getLineEcharts(function(response){
      let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('lineEcharts') as HTMLDivElement);
      let option={
        title:{
          text:responseData.title,
          subtext:'',
  ...
  createStackLineEcharts(){
    this.db.getStackLineEcharts(function(response){
      let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('stackLineEcharts') as HTMLDivElement);
      let option={
        title:{
          text:responseData.title,
          subtext:'',
  ...
  createGradientLineEcharts(){
    this.db.getGradientLineEcharts(function(response){
      let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('gradientLineEcharts') as HTMLDivElement);
      let option={
        title:{
          text:responseData.title,
          subtext:'',
        ...
        series:[
          {
             data:responseData.seriesData,
             type:'line',
             label:{show:true,//默认是false
              position:'top',fontSize:10},
             areaStyle:{normal:{
                color:new ECharts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'rgba(238,103,35,0.5)'},{offset:1,color:'rgba(238,103,35,0.00)'}])
              }
            } 
          }
        ]
  ...
  createCombinedEcharts(){
    this.db.getCombinedEcharts(function(response){
      let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('combinedEcharts') as HTMLDivElement);
      let option={
        title:{
          text:responseData.title,
          subtext:'',
  ...
  createRadarEcharts(){
    this.db.getRadarEcharts(function(response){
      //let responseData=JSON.parse(response);
      let myChart = ECharts.init(document.getElementById('radarEcharts') as HTMLDivElement);
      let option = {
        title: {
            text: '基础雷达图',
            textStyle:{
